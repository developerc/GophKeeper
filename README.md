...
Добавлен функционал сохранения данных в локальную БД SQLite. При штатной работе данные сохраняются и в PostgreSQL сервера, и в локальную БД SQLite клиента.  
В случае прерывания связи клиента с сервером запросы на добавление, изменение данных отклоняются. Разрешается только чтение данных из локальной БД SQLite.  
Переключение локального режима в штатный и наоборот происходит при запуске 20-го пункта меню клиента: Проверка соединения с gRPC сервером  
Если изменилось состояние связи, изменяется режим работы клиента.